...
class Loader:
    ...
    @staticmethod
    def create_student_params(model):
        return {'first_name': model.first_name, 'last_name': model.last_name, 'age': model.age}

    @staticmethod
    def create_teacher_params(model):
        params = {'salary': model.salary, 'degree': model.degree, 'workplace': model.workplace}
        return {**Loader.create_student_params(model), **params}

    def send(self, model):
        print(model)
        try:
            if type(model) is Student:
                params = {**Loader.create_student_params(model), **{'act': '0'}}
            else:
                params = {**Loader.create_teacher_params(model), **{'act': '1'}}
